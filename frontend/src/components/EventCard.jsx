      ...
      <div className="bg-white rounded-2xl border border-gray-200 overflow-hidden hover:shadow-lg hover:-translate-y-1 transition-all duration-200">
        ...
        <div className="relative h-40 bg-gradient-to-br from-indigo-500 to-purple-600">
          ...
          <div className="absolute top-3 left-3 bg-white rounded-xl px-3 py-1.5 text-center shadow">
            <div className="text-xs font-bold text-indigo-600 uppercase">{date.toLocaleString('en', { month: 'short' })}</div>
            <div className="text-lg font-black text-gray-900 leading-none">{date.getDate()}</div>
          </div>
          {event.is_free ? (
            <span className="absolute top-3 right-3 bg-green-500 text-white text-xs font-bold px-2.5 py-1 rounded-full">FREE</span>
          ) : (
            <span className="absolute top-3 right-3 bg-white text-indigo-700 text-xs font-bold px-2.5 py-1 rounded-full shadow">
              ৳{event.ticket_price}
            </span>
          )}
        </div>
        ...
        {/* Info */}
        <div className="p-4">
          <span className={`text-xs font-semibold px-2.5 py-0.5 rounded-full ${CATEGORY_COLORS[event.category] || CATEGORY_COLORS.other}`}>
            {event.category}
          </span>
    ...
    <Link to={`/lost-found/${item.id}`} className="group block">
      <div className="bg-white rounded-2xl border border-gray-200 overflow-hidden hover:shadow-lg hover:-translate-y-1 transition-all duration-200">
        <div className="relative h-44 bg-gray-100">
          ...
          <span className={`absolute top-2 left-2 text-xs font-bold px-2.5 py-1 rounded-full uppercase ${typeStyle}`}>
            {item.item_type}
          </span>
          {item.status === 'resolved' && (
            <div className="absolute inset-0 bg-black/40 flex items-center justify-center">
              <span className="bg-green-500 text-white font-bold px-4 py-1.5 rounded-full text-sm">RESOLVED</span>
            </div>
          )}
        </div>
        <div className="p-4">
