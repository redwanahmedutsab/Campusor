                ...
                <div className="relative h-48 bg-gray-100">
                    ...
                    {product.status === 'sold' && (
                        <div className="absolute inset-0 bg-black/50 flex items-center justify-center">
                            <span
                                className="bg-red-500 text-white font-bold px-4 py-1.5 rounded-full text-sm">SOLD</span>
                        </div>
                    )}
                    {product.is_free && (
                        <span
                            className="absolute top-2 left-2 bg-green-500 text-white text-xs font-bold px-2.5 py-1 rounded-full">FREE</span>
                    )}
                    <span
                        className={`absolute top-2 right-2 text-xs font-semibold px-2.5 py-1 rounded-full ${CONDITION_COLORS[product.condition] || 'bg-gray-100 text-gray-600'}`}>
