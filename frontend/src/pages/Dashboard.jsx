        ...
        <div className="min-h-screen bg-gray-50">
            <div className="max-w-5xl mx-auto px-4 py-8">
                ...
                <div className="bg-white rounded-2xl border border-gray-200 p-6 mb-6 flex flex-wrap items-center gap-4">
                    <div
                        className="w-16 h-16 rounded-full bg-indigo-600 text-white text-xl font-black flex items-center justify-center overflow-hidden shrink-0">
                    ...
                    <div className="flex-1">
                        ...
                        {user?.department &&
                            <p className="text-gray-400 text-xs mt-0.5">{user.department} · {user.university}</p>}
                    </div>
                ...
                {loading ? (
                    ...
                    tab === 'listings' && (
                        products.length === 0 ? (
                            <EmptyState icon="🛍️" text="No listings yet" sub="Post your first item to get started"
                                        action={() => navigate('/post?type=product')} actionLabel="Sell Item"/>
                        ) : (
                            <div className="space-y-3">
                                {products.map(p => (
                                    <div key={p.id}
                                        ...
                                        <div className="w-16 h-14 rounded-xl overflow-hidden bg-gray-100 shrink-0">
                                            {p.primary_image
                                                ...
                                                : <div
                                                    className="w-full h-full flex items-center justify-center text-2xl">📦</div>
                                            }
                                        </div>
                                        ...
                                        <div className="flex gap-2">
                                            {p.status === 'active' && (
                                                <button
                                                    onClick={() => markSold(p.id).then(() => setProducts(prev => prev.map(x => x.id === p.id ? {
                                                        ...x,
                                                        status: 'sold'
                                                    } : x)))}
                                            ...
                                            <button
                                                onClick={() => deleteProduct(p.id).then(() => setProducts(prev => prev.filter(x => x.id !== p.id)))}
                ...
                {!loading && tab === 'events' && (
                    events.length === 0 ? (
                        <EmptyState icon="🎉" text="No events organized" sub="Create your first campus event"
                                    action={() => navigate('/post?type=event')} actionLabel="Create Event"/>
                    ) : (
                        <div className="space-y-3">
                            {events.map(e => (
                                <div key={e.id}
                                     className="bg-white rounded-2xl border border-gray-200 p-4 flex items-center gap-4 cursor-pointer hover:shadow-sm"
                                     onClick={() => navigate(`/events/${e.id}`)}>
                                    <div
                                        className="w-12 h-12 rounded-xl bg-indigo-100 flex items-center justify-center text-xl shrink-0">🎉
                                    </div>
                                    <div className="flex-1 min-w-0">
                ...
                {!loading && tab === 'attending' && (
                    rsvps.length === 0 ? (
                        <EmptyState icon="✅" text="Not attending any events" sub="Browse and RSVP to campus events"
                                    action={() => navigate('/events')} actionLabel="Browse Events"/>
                    ) : (
                        <div className="space-y-3">
                            {rsvps.map(e => (
                                <div key={e.id}
                                     ...
                                     onClick={() => navigate(`/events/${e.id}`)}>
                                    <div
                                        className="w-12 h-12 rounded-xl bg-green-100 flex items-center justify-center text-xl shrink-0">✅
                                    </div>
                                    <div className="flex-1 min-w-0">
                                        <p className="font-semibold text-gray-900 truncate">{e.title}</p>
                                        <p className="text-xs text-gray-500">{new Date(e.start_datetime).toLocaleDateString()} · {e.location}</p>
                ...
                {!loading && tab === 'lostfound' && (
                    lfItems.length === 0 ? (
                        <EmptyState icon="🔍" text="No posts yet" sub="Report a lost or found item"
                                    action={() => navigate('/post?type=lostfound')} actionLabel="Post Item"/>
                    ) : (
                        <div className="space-y-3">
                            {lfItems.map(item => (
                                <div key={item.id}
                                    ...
                                    {item.status === 'open' && (
                                        <button onClick={() => markResolved(item.id).then(() =>
                                            setLfItems(prev => prev.map(x => x.id === item.id ? {
                                                ...x,
                                                status: 'resolved'
                                            } : x))
                                        )}
