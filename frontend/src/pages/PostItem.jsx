                        ...
                        {type === 'product' && (
                            <>
                                <div><label className={labelClass}>Title *</label>
                                    <input required value={product.title}
                                           onChange={e => setProduct(p => ({...p, title: e.target.value}))}
                                           className={inputClass} placeholder="What are you selling?"/>
                                    {fieldErr('title')}
                                </div>
                                <div><label className={labelClass}>Description *</label>
                                    <textarea required rows={4} value={product.description}
                                ...
                                <div className="grid grid-cols-2 gap-4">
                                    <div>
                                        ...
                                        <input type="number" required={!product.is_free} disabled={product.is_free}
                                               value={product.price}
                                    ...
                                    <div>
                                        ...
                                        <select value={product.condition}
                                                onChange={e => setProduct(p => ({...p, condition: e.target.value}))}
                                                className={inputClass}>
                                            {['new', 'like_new', 'good', 'fair', 'for_parts'].map(c => <option key={c}
                                                                                                               value={c}>{c.replace('_', ' ')}</option>)}
                                        </select>
                                    </div>
                                </div>
                                <label className="flex items-center gap-2 cursor-pointer">
                                    <input type="checkbox" checked={product.is_free} onChange={e => setProduct(p => ({
                                        ...p,
                                ...
                                <div>
                                    ...
                                    <select value={product.category}
                                        ...
                                        {categories.map(c => <option key={c.id}
                                                                     value={c.id}>{c.icon} {c.name}</option>)}
                                    </select>
                                </div>
                                <div><label className={labelClass}>Location / Where to meet</label>
                                    <input value={product.location}
                                           onChange={e => setProduct(p => ({...p, location: e.target.value}))}
                                           className={inputClass} placeholder="e.g. Main campus, Library"/>
                                </div>
                        ...
                        {type === 'event' && (
                            <>
                                <div><label className={labelClass}>Event Title *</label>
                                    <input required value={event.title}
                                           onChange={e => setEvent(p => ({...p, title: e.target.value}))}
                                           className={inputClass} placeholder="What's the event?"/>
                                </div>
                                <div><label className={labelClass}>Description *</label>
                                    <textarea required rows={4} value={event.description}
                                ...
                                <div className="grid grid-cols-2 gap-4">
                                    <div><label className={labelClass}>Category</label>
                                        <select value={event.category}
                                                onChange={e => setEvent(p => ({...p, category: e.target.value}))}
                                                className={inputClass}>
                                            {['academic', 'cultural', 'sports', 'tech', 'social', 'workshop', 'career', 'other'].map(c =>
                                                <option key={c} value={c} className="capitalize">{c}</option>)}
                                        </select>
                                    </div>
                                    <div><label className={labelClass}>Max Attendees</label>
                                        <input type="number" value={event.max_attendees}
                                               onChange={e => setEvent(p => ({...p, max_attendees: e.target.value}))}
                                               className={inputClass} placeholder="Leave blank for unlimited"/>
                                    </div>
                                </div>
                                <div className="grid grid-cols-2 gap-4">
                                    <div><label className={labelClass}>Start Date & Time *</label>
                                        <input type="datetime-local" required value={event.start_datetime}
                                               onChange={e => setEvent(p => ({...p, start_datetime: e.target.value}))}
                                               className={inputClass}/>
                                    </div>
                                    <div><label className={labelClass}>End Date & Time</label>
                                        <input type="datetime-local" value={event.end_datetime}
                                               onChange={e => setEvent(p => ({...p, end_datetime: e.target.value}))}
                                               className={inputClass}/>
                                    </div>
                                </div>
                                <div className="grid grid-cols-2 gap-4">
                                    <div><label className={labelClass}>Location *</label>
                                        <input required value={event.location}
                                               onChange={e => setEvent(p => ({...p, location: e.target.value}))}
                                               className={inputClass} placeholder="Campus / City"/>
                                    </div>
                                    <div><label className={labelClass}>Venue</label>
                                        <input value={event.venue}
                                               onChange={e => setEvent(p => ({...p, venue: e.target.value}))}
                                               className={inputClass} placeholder="e.g. Auditorium, Room 301"/>
                                    </div>
                                </div>
                                <label className="flex items-center gap-2 cursor-pointer">
                                    <input type="checkbox" checked={event.is_free}
                                           onChange={e => setEvent(p => ({...p, is_free: e.target.checked}))}
                                           className="rounded accent-indigo-600"/>
                                    <span className="text-sm font-medium text-gray-700">Free event</span>
                                </label>
                                {!event.is_free && (
                                    <div><label className={labelClass}>Ticket Price (BDT)</label>
                                        <input type="number" value={event.ticket_price}
                                               onChange={e => setEvent(p => ({...p, ticket_price: e.target.value}))}
                                               className={inputClass}/>
                                    </div>
                                )}
                                ...
                                <div><label className={labelClass}>Tags (comma-separated)</label>
                                    <input value={event.tags}
                                           onChange={e => setEvent(p => ({...p, tags: e.target.value}))}
                                           className={inputClass} placeholder="hackathon, coding, awards"/>
                                </div>
                            </>
                        )}
                            ...
                            <>
                                ...
                                <div><label className={labelClass}>Item Name *</label>
                                    <input required value={lf.title}
                                           onChange={e => setLf(p => ({...p, title: e.target.value}))}
                                           className={inputClass} placeholder="e.g. Blue backpack, Student ID"/>
                                </div>
                                <div><label className={labelClass}>Description *</label>
                                    <textarea required rows={3} value={lf.description}
                                              onChange={e => setLf(p => ({...p, description: e.target.value}))}
                                              className={inputClass} placeholder="Describe the item in detail..."/>
                                </div>
                                <div className="grid grid-cols-2 gap-4">
                                    <div><label className={labelClass}>Category</label>
                                        <select value={lf.category}
                                                onChange={e => setLf(p => ({...p, category: e.target.value}))}
                                                className={inputClass}>
                                            {['electronics', 'clothing', 'books', 'id_card', 'keys', 'bag', 'wallet', 'other'].map(c =>
                                                <option key={c} value={c}
                                                        className="capitalize">{c.replace('_', ' ')}</option>)}
                                        </select>
                                    </div>
                                    <div><label className={labelClass}>Date *</label>
                                        <input type="date" required value={lf.date_occurred}
                                               onChange={e => setLf(p => ({...p, date_occurred: e.target.value}))}
                                               className={inputClass}/>
                                    </div>
                                </div>
                                <div><label className={labelClass}>Location *</label>
                                    <input required value={lf.location}
                                           onChange={e => setLf(p => ({...p, location: e.target.value}))}
                                           className={inputClass} placeholder="Where was it lost/found?"/>
                                </div>
                                ...
                                <div className="border-t border-gray-100 pt-4">
                                    <p className="text-xs font-bold text-gray-500 uppercase tracking-wide mb-3">Contact
                                        Info (optional)</p>
                                    <div className="grid grid-cols-2 gap-4">
                                        <div><input placeholder="Your name" value={lf.contact_name}
                                                    onChange={e => setLf(p => ({...p, contact_name: e.target.value}))}
                                                    className={inputClass}/></div>
                                        <div><input placeholder="Phone number" value={lf.contact_phone}
                                                    onChange={e => setLf(p => ({...p, contact_phone: e.target.value}))}
                                                    className={inputClass}/></div>
                                    </div>
                                </div>
                            </>
