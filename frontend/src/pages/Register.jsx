...
const Register = () => {
  ...
  const fieldErr = (name) =>
    errors[name] ? <p className="text-red-500 text-xs mt-1">{Array.isArray(errors[name]) ? errors[name][0] : errors[name]}</p> : null;
        ...
        <div className="bg-white rounded-2xl border border-gray-200 shadow-sm p-8">
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div>
              ...
              <input name="username" required value={form.username} onChange={handleChange} className={inputClass} placeholder="min 4 characters" />
              {fieldErr('username')}
            </div>

            <div>
              <label className={labelClass}>Email *</label>
              <input type="email" name="email" required value={form.email} onChange={handleChange} className={inputClass} placeholder="your@university.edu" />
              {fieldErr('email')}
            </div>
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                ...
                <input name="student_id" value={form.student_id} onChange={handleChange} className={inputClass} />
              </div>
              <div>
            ...
            <div>
              ...
              <input name="university" value={form.university} onChange={handleChange} className={inputClass} placeholder="e.g. United International University" />
            </div>

            <div>
              <label className={labelClass}>Department</label>
              <input name="department" value={form.department} onChange={handleChange} className={inputClass} placeholder="e.g. Computer Science and Engineering" />
            </div>
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              <div>
                ...
                <input type="password" name="password2" required value={form.password2} onChange={handleChange} className={inputClass} />
                {fieldErr('password2')}
              </div>
            </div>
